...
namespace Bomberman
{
	...
	Player::Player(const SDL_Object* SDL_O) : Actor("paperella.png")
	{
		...
		NM = new NetworkManager("127.0.0.1", 8888);
		ID = NM->id;
		//position.x = 0;
		//position.y = 0;
		numberOfLives = 1;
		...
		NetworkOps = std::vector<NetworkOp>(4);
		
		NetworkOps[0] = &Player::UpdateOnlinePlayer;
		NetworkOps[1] = &Player::InstantiatePlayer;
		NetworkOps[2] = &Player::KillOnlinePlayer;
		NetworkOps[3] = &Player::InstantiateEgg;
	...
	void Player::Update() 
	{
		...
		rigidbody->velocity = velocity;
			...
			////SDL_Rect test2;
			////test2 = transform.GetRectangle();
			//
			//std::cout << "POS X " << transform.GetPosition().x << " | " << "POS Y " << transform.GetPosition().y << std::endl;
			//std::cout << "RECT X " << transform.GetRectangle().x << " | " << "RECT Y " << transform.GetRectangle().y << std::endl;
			////std::cout << "VEL X " << velocity.x << " | " << "VEL Y " << velocity.y << std::endl;
			...
			//transform.SDL_rect.x += testX;
			//transform.SDL_rect.y += testY;
	...
	void Player::InstantiateEgg(const Vector2 pos)
	{
		...
		unsigned char bufferX[sizeof(float)];
		unsigned char bufferY[sizeof(float)];
		byteconverter::IntToBytes(ID, Network_ID);
		byteconverter::FloatToBytes(bufferX, pos.x);
		byteconverter::FloatToBytes(bufferY, pos.y);
		byteconverter::IntToBytes(3, command);
		
		memcpy(message, Network_ID, sizeof(int));
		byteconverter::BytesAppend(message, 50, 4, command, sizeof(int));

		byteconverter::BytesAppend(message, 50, 8, bufferX, sizeof(float));
		byteconverter::BytesAppend(message, 50, 12, bufferY, sizeof(float));
		NM->send_to_server(message);
		//egg->SDL_rect.x = x;
		//egg->SDL_rect.y = y;
		//egg->SetCenter();
